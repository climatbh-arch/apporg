  ...
  updateQuote: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const result = await database.select().from(quotes).where(eq(quotes.id, input.id)).limit(1);
        const quote = result[0];
        if (!quote) throw new TRPCError({ code: "NOT_FOUND" });
        if (quote.userId !== (ctx.user?.id || 1)) throw new TRPCError({ code: "FORBIDDEN" });
  ...
  updateWorkOrder: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const result = await database.select().from(workOrders).where(eq(workOrders.id, input.id)).limit(1);
        const workOrder = result[0];
        if (!workOrder) throw new TRPCError({ code: "NOT_FOUND" });
        if (workOrder.userId !== (ctx.user?.id || 1)) throw new TRPCError({ code: "FORBIDDEN" });
